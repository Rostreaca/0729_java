...
public class Cafe {
	 ...
	 * 기능(method)를 구현하는 부분
	 * 
	 * 접근제한자 (예약어) 반환형(Return Type) 메소드식별자(매개변수) { 메소드 호출 시 실행할 코드; }
	 * 
	 * 접근제한자 : 호출할 수 있는 범위를 제한할 수 있음 반환형 : void => return할 때 돌려줄 값이 없음 : 반환 시 결과값의
	 * 자료형을 명시해준다. 매개변수 : 메소드 호출 시 전달 값을 받아주는 변수 메소드 범위에서만 사용 가능. 생략 가능.
	 * 
	 */
	public void open() {
		// dishWash();
		cleaning();
		windowCleaning();
		System.out.println(this.signatureMenuPrice);
	}

	// private void dishWash() { System.out.println("깨끗하게 빡빡!!"); }
	...
	/*
	 * DTO(Data Transfer Object) -> setter를 만들어줌 VO -> 일반적으로 setter 메소드를 구현하지 않음
	 * 
	 * DTO는 2달뒤에 배울건데 DTO학습 전까지만 VO클래스에 setter를 구현해서 사용
	 * 
	 * setter만들기 규칙
	 * 
	 * 1. setter메소드는 외부에서 접근이 가능해야 하기 때문에 접근제한자 public을 이용 2. set필드명으로 식별자를 작성하며
	 * 낙타봉표기법(camelCase)을 꼭 지키도록 한다.
	 * 
	...
	public void setSignatureMenuPrice(int signatureMenuPrice) {
		...
		// Scope안에서는 해당 영역 안에 있는 지역변수에 대한 우선권이 있음.
 ...
 */
	...
	*/
	...
	//getter() / setter() 다 만들었다 - > 캡슐화가 끝남(문법적인 관례)
	/*
	...
	public String info() {
		...
		String info = "[name 필드값  : " + name + ", address 필드 값 :" + address
				       + ", signatureMenuPrice : " + signatureMenuPrice+ "]";
